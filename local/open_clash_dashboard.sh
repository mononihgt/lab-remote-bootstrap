...
if [[ -z "$ENV_FILE" ]]; then
  if [[ -f "$ROOT_DIR/local/dashboard.env" ]]; then
    ENV_FILE="$ROOT_DIR/local/dashboard.env"
  elif [[ -f "$ROOT_DIR/host/host-stack.env" ]]; then
    ...
    echo "        bash local/open_clash_dashboard.sh local/dashboard.env"
    exit 1
  fi
fi
...
if [[ -z "${DISPLAY:-}" && "$(uname -s)" != "Darwin" ]]; then
  echo "[WARN] No GUI session detected. Tunnel will still be created; open dashboard URL manually."
fi
